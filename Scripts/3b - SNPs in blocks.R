...
for (i in 1:length(traits)){
  ...
  for(q in 1:length(envs)) {
    ...
    if (range(snips$p_wald,na.rm=T)[1]<thresh){
      ...
      sig.bins<-sig.bins %>% group_by(hapID) %>% summarise(NSNP=length(ps),beta=max(beta),min_p=min(p_wald), RES=max(RES))
    ...
    sug.bins<-sug.bins %>% group_by(hapID) %>% summarise(NSNP=length(ps),beta=max(beta),min_p=min(p_wald), RES=max(RES))
