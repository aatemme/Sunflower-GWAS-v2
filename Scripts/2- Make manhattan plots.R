...
for (i in 1:length(traits)){
  ...
  pdf(paste("Plots/Manhattans/",traits[i],"_ManhattanPlot.pdf",sep=""),height=11,width=10)
  ...
  for (q in 1:length(envs)){
      ...
      manhattan(ps1[ps1$p_wald<0.01,],chr = "Chr_num", bp = "ps", p = "p_wald", snp = "rs",col=brewer.pal(8,"Dark2")[c(1,8)],
                suggestiveline = -log10(tmpcutoff),genomewideline=-log10(0.05/31733), cex=0.5, cex.axis=0.8,ylim=c(2,ytop),
