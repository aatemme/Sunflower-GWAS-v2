...
#### add block info to sig snips
sig.snips$hapID<-big.list$hapID[match(sig.snips$rs,big.list$SNP)]

hapID.sigsnips<-unique(sig.snips$hapID)


snps.in.sig.hapID<-big.list$SNP[big.list$hapID%in%hapID.sigsnips]

### rerun haplotype analyses on just significant snps
exclude<-big.list$SNP[!big.list$hapID%in%hapID.sigsnips]
write.table<-write.table(exclude, "Tables/Blocks/snps_NOT_in_sig_blocks.txt", sep="\t", row.names=F, col.names=T, quote=F)
...
missing.snps<-all.snps[all.snps$rs%in%(snps.in.sig.hapID[!snps.in.sig.hapID%in%sig.list$SNP]), ]
missing.snps$Chr_num<- as.integer(gsub("Ha412HOChr","",missing.snps$chr))
...
#### filter back to just significant snps
sig.list<-sig.list[sig.list$SNP%in%sig.snips$rs,]
